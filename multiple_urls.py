...
class FileDownloaderApp:
    def __init__(self, root: tk.Tk):
        ...
        self.textbox = tk.Text(self.file_url_frame, height=5, width=60)
        ...
        ######### Checksum Frame #########
        self.checksum_frame = tk.LabelFrame(self.root, text="Checksum (SHA256)")
        self.checksum_frame.pack(fill="both", expand="yes", padx=10, pady=10)

        self.checksum_label = tk.Label(self.checksum_frame, text="Expected Checksum (one per line, optional):")
        self.checksum_label.grid(row=0, column=0, padx=10, pady=10)
        self.checksum_textbox = tk.Text(self.checksum_frame, height=5, width=60)
        self.checksum_textbox.grid(row=1, column=0, padx=10, pady=10)
    ...
    def start_downloads(self):
        ...
        checksums = self.checksum_textbox.get("1.0", tk.END).strip().split("\n")

        for i, url in enumerate(urls):
            checksum = checksums[i] if i < len(checksums) else None
    ...
    def download_file_thread(self, download_info):
        ...
        try:
            ...
            with open(file_name, "ab") as f:
                ...
                if not download_info["stop_event"].is_set():
                    download_info["elapsed_time"].set(f"{round(time.perf_counter() - start)} seconds")
                    self.update_listbox(download_info)
                    if self.verify_checksum(file_name, download_info["checksum"]):
                        messagebox.showinfo("Success", f"File {os.path.basename(file_name)} downloaded and checksum verified successfully.")
                    else:
                        messagebox.showerror("Error", f"Checksum verification failed for {os.path.basename(file_name)}!")
                else:
                    messagebox.showinfo("Stopped", f"Download for {os.path.basename(file_name)} has been stopped.")
